...
export async function GET(req: Request) {
  ...
  // If a specific questionId is provided, return results for that question only
  if (questionId) {
    const question = storage.getQuestion(roundId, questionId)
    if (!question) {
      return NextResponse.json({ error: 'question_not_found' }, { status: 404 })
    }
    const results = computeQuestionResults(roundId, questionId, question.correctOption)
  ...
  // Return results for all questions
